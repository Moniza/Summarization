...
public class TF3IDF1 {

	int collectionSize = 0;

	public String RetriveBookId(String Bname) throws IOException {
		...
		/*
		 * while((str=br.readLine())!=null) { String
		 * lowercase=str.toLowerCase(); String[] BookId=str.split(":");
		 * if(BookId[0].contains(Bname)) { return BookId[1]; }
		 * 
		 * 
		 * }
		 * 
	...
	public String ExtractBookReview(String Bname) throws IOException {
		File f = new File("D://summary/sentenceArrangement/");
	...
	public Map<String, String> ExtractReviewTermsFromIndex(String BookID)
			throws IOException {
		int max=0;
		Map<String, String> map_Term_DF_TF = new HashMap<>();
		FileInputStream fr = new FileInputStream("D://summary/Index/Index.txt");
		DataInputStream in = new DataInputStream(fr);
		BufferedReader br = new BufferedReader(new InputStreamReader(in));
		String str;
		while ((str = br.readLine()) != null) {
			...
			if (term_DTf[1].contains(BookID)) {
				...
				int DF = a.length - 1;
				String[] temp1 = term_DTf[1].split("D[0-9]T");
				for (int i = 1; i < temp1.length; i++) {
					 max = Integer.parseInt(temp1[1]);
					if (max < Integer.parseInt(temp1[i])) 
							max=Integer.parseInt(temp1[i]);
				...
				while (match.find()) {
					...
					String newString = term_DTf[1].substring(match.start(),
							match.end());
					// System.out.println(newString);
					String[] TF = newString.split(BookID + "T");
					map_Term_DF_TF.put(term_DTf[0], DF + ":" + TF[1]+":"+max);
	...
	public Map<String, Float> ComputeTFIDF(Map  map_Term_DF_TF)
	{
		...
		while (it.hasNext()) {
			...
			float max=Float.parseFloat(DF0TF1[2]);
			//System.out.println(max);
			float TF=(float)(0.5+((0.5*temp)/max));
			float IDF=(float) (1+Math.log(collectionSize/DF));
			float tf_idf=TF*IDF;
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println("Book name:");// name of book whose review user want
											// to get.
		String Bname = "da-vinci-code-Review.txt";
		TF3IDF1 ob1 = new TF3IDF1();
		String bookReview = ob1.ExtractBookReview(Bname);
		Map sortedTerms = ob1.sort(ob1.ComputeTFIDF(ob1
				.ExtractReviewTermsFromIndex(ob1.RetriveBookId(Bname))));
